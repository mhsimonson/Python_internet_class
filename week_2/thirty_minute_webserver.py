...
def get_content(uri):
    ...
    try:
        path = '.' + uri
        print path
## Help from Brian - test code to understand the interchange between server and shell
#        
#        if path=='./time/':
#            print "time path"
...
## /Help from Brian
        if path=='./time/':
#           p=subprocess.Popen(['python', '/Users/mark/Python_internet_class/thirty_minute_app/time/mhs_print_time_try.py'], bufsize=140000, stdout=subprocess.PIPE)
#  Original file above, modified below for use in other directories.
            print os.getcwd()
            p=subprocess.Popen(['python', os.getcwd()+uri+'\print_time_mhs.py'], stdout=subprocess.PIPE)
            p.wait()
            time_output=p.stdout.read()
            print time_output
            return (200, 'text/html', time_output)
